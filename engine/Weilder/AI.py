    ...
    #face_save
        {
            ...
            "function": {
                "name": "User_name",
                "description": "saves user's name that is provided by the user",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "a": {
...
def core(prompt,person,permi):
    ...
    system_message = {
            "role": "system",
            "content": f"""
                        Your name is AiTOMM pronounced as Atom. Answer in simple and short sentences;
                        ensure the person infont of you is {permi} and he is sarounded with {objects}
                        current time is: {time.strftime('%Y-%m-%d %H:%M:%S')},and {temparature()}.
                        and adapt to the culture of {location()} but never respond in said language unless asked default to english.
                        . Do not let memory overshadow the user's prompt.You can use tools to perform specific tasks, 
                        such as creating files or controlling apps including command {protocall(prompt)}.
                        If a tool is relevant to the user's request, call the tool instead of generating a direct answer.
                        """
    }

    messages = [system_message]  
    messages.extend(loadmemory(str(person[0]))) 
...
def mainframe(text):
    ...
    print(person) 
    perim = impretion(person)
    print(f"<----------------------------{person}----------------------->")

    content = f"{text} " 
    text = core(text,person,perim)
    
    save_conversation(content, text,str(person[0]))
    text = re.sub(r'".*?"', '', text)
